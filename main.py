...
company_income = st.sidebar.number_input(label=f'Please enter your income here (£):',value=88800, step=1000)
st.sidebar.subheader('Salary')
salary = st.sidebar.number_input(label=f'Please enter your salary here (£):',value=12570, step=1000)
st.sidebar.subheader('Expenses')
expenses = st.sidebar.number_input(label=f'Please enter your expenses here (£):',value=2000, step=100)

########################################################################################################################

# Income Tax Rates
personal_allowance = 12570 #the allowed "zero" tax rate that is allowed for all salaries
basic_rate_bracket = 37700 #the brackets for which the basic, higher and additional rate applies
...
if taxable_salary <= personal_allowance:
    ...
    income_tax = float(basic_rate_bracket * basic_rate_tax) + float((taxable_salary - basic_rate_bracket) * higher_rate_tax)
elif taxable_salary >= (higher_rate_bracket + 1):
    income_tax = float(basic_rate_tax * basic_rate_bracket) + float(higher_rate_bracket * higher_rate_tax) + float((taxable_salary - higher_rate_bracket) * additional_rate_tax)
else:
    income_tax = 0
...
if taxable_salary <= class_1_lower_bracket:
    ...
    NI_contribution = float((class_1_lower_bracket - NI_threshold) * class_1_lower_tax_rate) + float((taxable_salary - class_1_lower_bracket) * class_1_higher_tax_rate)
else:
    NI_contribution = 0
...
if Dividend_amount <= basic_rate_bracket:
    ...
    Dividend_tax = float(basic_rate_bracket * Dividend_basic_rate_tax) + float((Dividend_amount - basic_rate_bracket) * Dividend_higher_rate_tax)
elif Dividend_amount >= (higher_rate_bracket + 1):
    Dividend_tax = float(basic_rate_tax * Dividend_basic_rate_tax) + float(higher_rate_bracket * Dividend_higher_rate_tax) + float((Dividend_amount - higher_rate_bracket) * Dividend_additional_rate_tax)
else:
...
Interest_rate_text = st.sidebar.subheader(round(Corporation_rate_tax*100,))
...
st.markdown(f'Take-home dividend pay = £{round(((company_income-Total_tax)-(salary-income_tax))/12,):,}')
...
st.subheader(f'Thats **{round(((company_income-Total_tax)/12)/((company_income/12))*100):,}%**')
